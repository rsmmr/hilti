...
typedef void* binpac_parse_function(hlt_bytes* data, void* user, hlt_exception** excpt,
                                    hlt_execution_context* ctx);
typedef void* binpac_resume_function(hlt_exception* yield, hlt_exception** excpt,
                                     hlt_execution_context* ctx);
typedef void binpac_compose_output_function(hlt_bytes* data, void** obj, hlt_type_info* type,
                                            void* user, hlt_exception** excpt,
                                            hlt_execution_context* ctx);
typedef void binpac_compose_function(void* pobj, binpac_compose_output_function* output, void* user,
                                     hlt_exception** excpt, hlt_execution_context* ctx);

// Internal functions for parsing from sink.write.
typedef void __binpac_parse_sink_function(void* pobj, hlt_bytes* data, void* user,
                                          hlt_exception** excpt, hlt_execution_context* ctx);
typedef void __binpac_resume_sink_function(hlt_exception* yield, hlt_exception** excpt,
                                           hlt_execution_context* ctx);

// Internal function to create an instance of a parser.
struct binpac_sink;
typedef void* __binpac_new_function(struct binpac_sink* sink, hlt_bytes* mimetype, int8_t try_mode,
                                    void* cookie, hlt_exception** excpt,
                                    hlt_execution_context* ctx);

// Internal functions to trigger specific hooks.
typedef void __binpac_gap_hook(void* pobj, void* cookie, uint64_t seq, uint64_t len,
                               hlt_exception** excpt, hlt_execution_context* ctx);
typedef void __binpac_skip_hook(void* pobj, void* cookie, uint64_t seq, hlt_exception** excpt,
                                hlt_execution_context* ctx);
typedef void __binpac_overlap_hook(void* pobj, void* cookie, uint64_t seq, hlt_bytes* b1,
                                   hlt_bytes* b2, hlt_exception** excpt,
                                   hlt_execution_context* ctx);
typedef void __binpac_undelivered_hook(void* pobj, void* cookie, uint64_t seq, hlt_bytes* b,
                                       hlt_exception** excpt, hlt_execution_context* ctx);
...
typedef struct __binpac_parser {
    ...
    __binpac_parse_sink_function* parse_func_sink;   // The C function performing the parsing for
                                                     // sink.write. For internal use only.
    __binpac_resume_sink_function* resume_func_sink; // The C function resuming sink parsing after a
                                                     // yield. For internal use only.
    __binpac_new_function* new_func; // C function to create a new instance of the parser. May be
