...
binpac_unit_cookie binpac_unit_iterate(binpac_unit_item* dst, const hlt_type_info* type, void* unit, int flags, binpac_unit_cookie idx, hlt_exception** excpt, hlt_execution_context* ctx)
{
    if ( ! idx )
        // First call.
    ...
    binpac_type_id btype = *(binpac_type_id*)hlt_tuple_get(type->hostapp_type, type->hostapp_value, 0, excpt, ctx);
