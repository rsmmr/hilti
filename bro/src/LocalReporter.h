...
inline void push_location(const std::string& file, int line)
{
    __push_location(file.c_str(), line);
}
inline void pop_location()
{
    __pop_location();
}
...
{
    __fatal_error(msg.c_str());
}
inline void internal_error(const std::string& msg)
{
    __internal_error(msg.c_str());
}
inline void weird(Connection* conn, const std::string& msg)
{
    __weird(conn, msg.c_str());
}
inline void weird(const std::string& msg)
{
    __weird(0, msg.c_str());
}
