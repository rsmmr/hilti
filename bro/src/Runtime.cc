...
int8_t libbro_cookie_to_is_orig_boolean(void* cookie, hlt_exception** excpt,
...
::Val* libbro_h2b_bytes(hlt_bytes* b, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    const int len = hlt_bytes_len(b, excpt, ctx);
    unsigned char* data = new unsigned char[len];
    hlt_bytes_to_raw((int8_t*)data, len, b, excpt, ctx);
    BroString* s = new BroString(data, len, 1);
    delete[] data;
...
::Val* libbro_h2b_enum(const hlt_type_info* type, void* obj, uint64_t type_idx,
                       hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    return hlt_enum_has_val(e) ? new EnumVal(hlt_enum_value(e, excpt, ctx), et) :
                                 new EnumVal(lib_bro_enum_undef_val, et);
}

::Val* libbro_h2b_enum_type(const hlt_type_info* type, void* obj, ::EnumType* etype,
                            hlt_exception** excpt, hlt_execution_context* ctx)
{
    hlt_enum e = *((hlt_enum*)obj);

    return hlt_enum_has_val(e) ? new EnumVal(hlt_enum_value(e, excpt, ctx), etype) :
                                 new EnumVal(lib_bro_enum_undef_val, etype);
}
...
hlt_bytes* libbro_b2h_string(Val* val, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    hlt_bytes* result =
        hlt_bytes_new_from_data_copy((const int8_t*)s->Bytes(), s->Len(), excpt, ctx);
    return result;
}
...
hlt_file* libbro_b2h_file(Val* val, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    hlt_file_open(hfile, fname, Hilti_FileType_Text, Hilti_FileMode_Append, Hilti_Charset_UTF8,
                  excpt, ctx);
...
void* libbro_get_event_handler(hlt_bytes* name, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    char evname[256];
    hlt_bytes_to_raw((int8_t*)evname, sizeof(evname), name, excpt, ctx);
    evname[len] = '\0';
...
void libbro_raise_event(void* hdl, const hlt_type_info* type, void* tuple, hlt_exception** excpt,
                        hlt_execution_context* ctx)
{
...
::Val* libbro_call_legacy_result(::Val* val, const hlt_type_info* type, void* tuple,
                                 hlt_exception** excpt, hlt_execution_context* ctx)
{
...
void libbro_call_legacy_void(::Val* func, const hlt_type_info* type, void* tuple,
                             hlt_exception** excpt, hlt_execution_context* ctx)
{
...
::BroType* libbro_bro_lookup_type(hlt_string name, hlt_exception** excpt,
                                  hlt_execution_context* ctx)
{
    ...
    if ( ! id )
        bro::hilti::reporter::internal_error(
            ::util::fmt("libbro_lookup_type: unknown type '%s'", n));

    ::BroType* t = id->AsType();

    if ( ! t )
        bro::hilti::reporter::internal_error(
            ::util::fmt("libbro_lookup_type: ID '%s' is not a type", n));
...
::Val* libbro_bro_lookup_type_as_val(hlt_string name, hlt_exception** excpt,
                                     hlt_execution_context* ctx)
{
...
::Val* libbro_bro_lookup_id_as_val(hlt_string name, hlt_exception** excpt,
                                   hlt_execution_context* ctx)
{
    auto n = hlt_string_to_native(name, excpt, ctx);
    ::ID* id = ::global_scope()->Lookup(n);
    ...
    if ( ! id )
        bro::hilti::reporter::internal_error(
            ::util::fmt("libbro_lookup_id_as_val: unknown ID '%s'", n));

    auto val = id->ID_Val();

    if ( ! val )
        bro::hilti::reporter::internal_error(
            ::util::fmt("libbro_lookup_id_as_val: ID %s does not have a value", n));
...
::TableType* libbro_bro_table_type_new(::BroType* key, ::BroType* value, hlt_exception** excpt,
                                       hlt_execution_context* ctx)
{
    return new ::TableType(key->AsTypeList(), value);
}

::TableVal* libbro_bro_table_new(::TableType* type, hlt_exception** excpt,
                                 hlt_execution_context* ctx)
{
    return new ::TableVal(type);
}

void libbro_bro_table_insert(::TableVal* val, ::Val* k, ::Val* v, hlt_exception** excpt,
                             hlt_execution_context* ctx)
{
    if ( v )
        Ref(v);
...
void libbro_bro_list_append(::ListVal* lval, ::Val* val, hlt_exception** excpt,
                            hlt_execution_context* ctx)
{
    if ( val )
        Ref(val);
...
::TypeList* libbro_bro_list_type_new(::BroType* pure_type, hlt_exception** excpt,
                                     hlt_execution_context* ctx)
{
    return new ::TypeList(pure_type);
}

void libbro_bro_list_type_append(::TypeList* t, ::BroType* ntype, hlt_exception** excpt,
                                 hlt_execution_context* ctx)
{
    assert(ntype);
    Ref(ntype);
...
::RecordVal* libbro_bro_record_new(::RecordType* rtype, hlt_exception** excpt,
                                   hlt_execution_context* ctx)
{
...
void libbro_bro_record_assign(::RecordVal* rval, uint64_t idx, ::Val* val, hlt_exception** excpt,
                              hlt_execution_context* ctx)
{
    if ( ! val )
        return;
...
::RecordType* libbro_bro_record_type_new(hlt_list* fields, hlt_exception** excpt,
                                         hlt_execution_context* ctx)
{
...
::TypeDecl* libbro_bro_record_typedecl_new(hlt_string fname, ::BroType* ftype,
                                           hlt_exception** excpt, hlt_execution_context* ctx)
{
...
::VectorVal* libbro_bro_vector_new(::VectorType* vtype, hlt_exception** excpt,
                                   hlt_execution_context* ctx)
{
    return new VectorVal(vtype);
}

void libbro_bro_vector_append(::VectorVal* vval, ::Val* val, hlt_exception** excpt,
                              hlt_execution_context* ctx)
{
    if ( val )
        Ref(val);
...
::VectorType* libbro_bro_vector_type_new(::BroType* ytype, hlt_exception** excpt,
                                         hlt_execution_context* ctx)
{
...
::EnumType* libbro_bro_enum_type_new(hlt_string module, hlt_string name, hlt_exception** excpt,
                                     hlt_execution_context* ctx)
{
    auto n = hlt_string_to_native(name, excpt, ctx);
...
void libbro_bro_enum_type_add_name(::EnumType* etype, hlt_string module, hlt_string name,
                                   uint64_t val, hlt_exception** excpt, hlt_execution_context* ctx)
{
    auto n = hlt_string_to_native(name, excpt, ctx);
    auto m = hlt_string_to_native(module, excpt, ctx);
...
::FuncType* libbro_bro_function_type_new(::BroType* args, ::BroType* ytype, int64_t flavor,
                                         hlt_exception** excpt, hlt_execution_context* ctx)
{
    if ( args )
        Ref(args);
...
void libbro_object_mapping_register(void* bobj, hlt_type_info* ti, void** hobj,
                                    hlt_exception** excpt, hlt_execution_context* ctx)
{
    assert(bobj);
...
void libbro_object_mapping_unregister_bro(void* obj, hlt_exception** excpt,
                                          hlt_execution_context* ctx)
{
...
void libbro_object_mapping_invalidate_bro(void* obj, hlt_exception** excpt,
                                          hlt_execution_context* ctx)
{
    libbro_object_mapping_unregister_bro(obj, excpt, ctx);
}

void libbro_object_mapping_unregister_hilti(hlt_type_info* ti, void** hobj, hlt_exception** excpt,
                                            hlt_execution_context* ctx)
{
    assert(false);
}

const ::BroObj* libbro_object_mapping_lookup_bro(hlt_type_info* ti, void** obj,
                                                 hlt_exception** excpt, hlt_execution_context* ctx)
{
...
void* libbro_object_mapping_lookup_hilti(::BroObj* obj, hlt_exception** excpt,
                                         hlt_execution_context* ctx)
{
...
void bro_file_set_size(uint64_t size, void* cookie, hlt_exception** excpt,
                       hlt_execution_context* ctx)
{
...
void bro_file_data_in(hlt_bytes* data, void* cookie, hlt_exception** excpt,
                      hlt_execution_context* ctx)
{
    ...
    while ( true ) {
        ...
        file_mgr->DataIn((const u_char*)block.start, block.end - block.start, c->tag,
...
void bro_file_data_in_at_offset(hlt_bytes* data, uint64_t offset, void* cookie,
                                hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    while ( true ) {
        ...
        file_mgr->DataIn((const u_char*)block.start, block.end - block.start, offset, c->tag,
                         c->analyzer->Conn(), c->is_orig, _file_id(c));
...
void bro_file_gap(uint64_t offset, uint64_t len, void* cookie, hlt_exception** excpt,
                  hlt_execution_context* ctx)
{
...
void bro_rule_match(hlt_enum pattern_type, hlt_bytes* data, int8_t bol, int8_t eol, int8_t clear,
                    void* cookie, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    while ( true ) {
        ...
        c->analyzer->Conn()->Match(bro_type, (const u_char*)block.start, block.end - block.start,
                                   c->is_orig, first ? bol : false, bcookie ? false : eol,
                                   first ? clear : false);
...
int8_t bro_get_const_bool(hlt_string id, void* cookie, hlt_exception** excpt,
                          hlt_execution_context* ctx)
{
...
typedef struct hlt_LibBro_BroAny* (*from_val_func_t)(Val*, hlt_exception** excpt,
                                                     hlt_execution_context* ctx);
typedef Val* (*to_val_func_t)(struct hlt_LibBro_BroAny*, hlt_exception** excpt,
                              hlt_execution_context* ctx);
...
hlt_LibBro_BroAny* libbro_any_from_hilti_ref(hlt_type_info* ti, void* obj, BroType* btype,
                                             to_val_func_t to_val_func, hlt_exception** excpt,
                                             hlt_execution_context* ctx)
{
