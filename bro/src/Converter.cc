...
void TypeConverter::visit(::hilti::type::Enum* e)
{
    ...
    if ( module.empty() || name.empty() )
        reporter::internal_error(::util::fmt(
            "TypeConverter::visit(Enum): cannot determine type's module/name (%s/%s|%s)",
    ...
    eresult->AddName(module, undef.c_str(), lib_bro_enum_undef_val, true, false);
    ...
    for ( auto l : etype->labels() ) {
        ...
        eresult->AddName(module, name.c_str(), l.second, true, false);
    }
...
void TypeConverter::visit(::hilti::type::Tuple* t)
{
    ...
    if ( auto bmtype = ast::rtti::tryCast<spicy::type::Bitfield>(arg1()) ) {
        ...
        for ( auto m : ::util::zip2(t->typeList(), bmtype->bits()) ) {
            ...
            auto td = new ::TypeDecl(Convert(m.first, m.second->fieldType()),
                                     copy_string(name.c_str()), 0, true);
            tdecls->append(td);
        }
    }
        ...
        for ( auto m : ::util::zip2(t->typeList(), btype->typeList()) ) {
            ...
            auto td =
                new ::TypeDecl(Convert(m.first, m.second), copy_string(name.c_str()), 0, true);
            tdecls->append(td);
            ++n;
            ++i;
        }
