...
analyzer::Tag plugin::Bro_Hilti::Plugin::AddAnalyzer(const string& name, TransportProto proto,
                                                     analyzer::Tag::subtype_t stype)
{
    PLUGIN_DBG_LOG(HiltiPlugin, "Adding analyzer %s", name.c_str());
...
std::pair<bool, Val*> plugin::Bro_Hilti::Plugin::HookCallFunction(const Func* func, Frame* parent,
                                                                  val_list* args)
{
    ...
    if ( func->FType()->Flavor() == FUNC_FLAVOR_EVENT && _manager->HaveCustomHandler(func) ) {
        // We don't own the arguments so ref them all once more.
        loop_over_list(*args, i) Ref((*args)[i]);
