...
{
    if ( (! id->ID_Val()) && id->Type()->Tag() == TYPE_FUNC &&
         compiler->HaveHiltiBif(id->Name()) ) {
...
TraversalCode NormalizerCallBack::PreExpr(const ::Expr* expr)
{
    if ( expr->Tag() == EXPR_CONST ) {
        auto val = static_cast<const ::ConstExpr*>(expr)->Value();
        assert(val);
...
std::list<const ::Func*> CollectorCallback::Functions(const string& ns)
{
    ...
    functions.sort(
        [](const Func* a, const Func* b) -> bool { return strcmp(a->Name(), b->Name()) < 0; });
...
std::list<const ::ID*> CollectorCallback::Globals(const string& ns)
{
    ...
    globals.sort(
        [](const ::ID* a, const ::ID* b) -> bool { return strcmp(a->Name(), b->Name()) < 0; });
...
Compiler::module_list Compiler::CompileExternalModules()
{
    ...
    for ( auto path : external_modules ) {
        ...
        for ( int i = 0; i < handlers->length(); ++i ) {
            ...
            auto symbol =
                HiltiSymbol(::util::fmt("%s::%s", mod, name), true, m->id()->name(), false);
...
std::shared_ptr<::hilti::Module> Compiler::FinalizeGlueBuilder()
{
    if ( BifConst::Hilti::save_hilti )
        glue_module_builder->saveHiltiCode(
            ::util::fmt("bro.%s.hlt", glue_module_builder->module()->id()->name()));
...
string Compiler::HiltiSymbol(const ::Func* func, shared_ptr<::hilti::Module> module,
                             bool include_module)
{
...
std::string Compiler::HiltiStubSymbol(const ::Func* func, shared_ptr<::hilti::Module> module,
                                      bool include_module)
{
    return normalizeSymbol(func->Name(), "", "stub", module ? module->id()->name() : "", true,
                           include_module);
...
string Compiler::HiltiSymbol(const std::string& id, bool global, const std::string& module,
                             bool include_module)
{
    return normalizeSymbol(id, "", "", module, global, include_module);
}

std::string Compiler::normalizeSymbol(const std::string sym, const std::string prefix,
                                      const std::string postfix, const std::string& module,
                                      bool global, bool include_module)
{
...
std::pair<bool, ::Func*> Compiler::BroExprToFunc(const ::Expr* func)
{
    if ( func->Tag() == EXPR_NAME ) {
        ...
        auto ee = static_cast<const ::EventExpr*>(func);
        ...
        if ( ::global_scope()->Lookup(ee->Name()) )
            ...
            Error(
                func,
                ::util::fmt(
                    "event %s is not a global ID (indirect event operands aren't supported by Bro)",
                    ee->Name())
                    .c_str());
        ...
        auto cexpr = static_cast<const ::ConstExpr*>(func);
...
void Compiler::CacheCustomBroType(const BroType* btype, shared_ptr<::hilti::Type> htype,
                                  const std::string& bro_id_name)
{
    cached_custom_types.insert(std::make_pair(btype, std::make_pair(htype, bro_id_name)));
}

std::pair<shared_ptr<::hilti::Type>, std::string> Compiler::LookupCachedCustomBroType(
    const BroType* btype)
{
