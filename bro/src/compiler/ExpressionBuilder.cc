...
#define UNARY_OP_FUNC                                                                              \
    [&](const ::UnaryExpr* expr, shared_ptr<::hilti::Expression> dst,                              \
        shared_ptr<::hilti::Expression> op)

#define BINARY_OP_FUNC                                                                             \
    [&](const ::BinaryExpr* expr, shared_ptr<::hilti::Expression> dst,                             \
...
ExpressionBuilder::ExpressionBuilder(class ModuleBuilder* mbuilder) : BuilderBase(mbuilder)
{
}

std::shared_ptr<::hilti::Expression> ExpressionBuilder::NotSupported(const ::BroType* type,
                                                                     const char* where)
{
    ODesc d;
    type->Describe(&d);

    Error(::util::fmt("type %s not supported in %s (%s)", type_name(type->Tag()), where,
                      d.Description()),
          type);
    return nullptr;
}

std::shared_ptr<::hilti::Expression> ExpressionBuilder::NotSupported(const ::Expr* expr,
                                                                     const char* where)
{
    ODesc d;
    expr->Describe(&d);

    Error(::util::fmt("expression %s not supported in %s (%s)", expr_name(expr->Tag()), where,
                      d.Description()),
          expr);
    return nullptr;
}

std::shared_ptr<::hilti::Expression> ExpressionBuilder::NotSupported(const ::UnaryExpr* expr,
                                                                     const char* where)
{
    ODesc d;
    expr->Describe(&d);
    ...
    Error(::util::fmt("unary expression %s not supported for operand of type %s in %s (%s)",
                      ::expr_name(expr->Tag()), ::type_name(expr->Op()->Type()->Tag()), where,
                      d.Description()),
          expr);
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::NotSupported(const ::BinaryExpr* expr,
                                                                     const char* where)
{
    ODesc d;
    expr->Describe(&d);
    ...
    Error(::util::
              fmt("binary expression %s not supported for operands of types %s and %s in %s (%s)",
                  ::expr_name(expr->Tag()), ::type_name(expr->Op1()->Type()->Tag()),
                  ::type_name(expr->Op2()->Type()->Tag()), where, d.Description()),
          expr);
...
bool ExpressionBuilder::CompileOperator(const ::Expr* expr, shared_ptr<::hilti::Expression> dst,
                                        ::TypeTag op, shared_ptr<::hilti::Instruction> ins,
...
bool ExpressionBuilder::CompileOperator(const ::UnaryExpr* expr,
                                        shared_ptr<::hilti::Expression> dst, ::TypeTag op,
...
bool ExpressionBuilder::CompileOperator(const ::BinaryExpr* expr,
                                        shared_ptr<::hilti::Expression> dst, ::TypeTag op1,
                                        ::TypeTag op2, shared_ptr<::hilti::Instruction> ins,
                                        shared_ptr<::hilti::Expression> addl_op)
{
...
bool ExpressionBuilder::CompileOperator(const ::BinaryExpr* expr,
                                        shared_ptr<::hilti::Expression> dst, ::TypeTag op1,
                                        ::TypeTag op2, binary_op_func func)
{
    if ( expr->Op1()->Type()->Tag() != op1 || expr->Op2()->Type()->Tag() != op2 )
        return false;
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::AddExpr* expr)
{
    ...
    ok = ok ||
         CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT, ::hilti::instruction::integer::Add);
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                               ::hilti::instruction::integer::Add);
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                               ::hilti::instruction::double_::Add);
    ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_INTERVAL,
                               ::hilti::instruction::time::Add);
    ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
                               ::hilti::instruction::interval::Add);
    ok = ok || CompileOperator(expr, result, ::TYPE_STRING, ::TYPE_STRING,
                               ::hilti::instruction::bytes::Concat);
...
shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAddTo(const ::Expr* expr,
                                                                ...
                                                                bool add)
{
    ...
    if ( add ) {
        ...
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_COUNT, ::hilti::instruction::integer::Add, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_INTERVAL, ::hilti::instruction::interval::Add, val);
        ok = ok || CompileOperator(expr, nval, ::TYPE_TIME, ::hilti::instruction::time::Add, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_DOUBLE, ::hilti::instruction::double_::Add, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_STRING, ::hilti::instruction::bytes::Concat, val);
    }

    else {
        ok = ok || CompileOperator(expr, nval, ::TYPE_INT, ::hilti::instruction::integer::Sub, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_COUNT, ::hilti::instruction::integer::Sub, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_INTERVAL, ::hilti::instruction::interval::Sub, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_TIME, ::hilti::instruction::time::SubTime, val);
        ok = ok ||
             CompileOperator(expr, nval, ::TYPE_DOUBLE, ::hilti::instruction::double_::Sub, val);
    }
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::ArithCoerceExpr* expr)
{
    ...
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, UNARY_OP_FUNC {
             switch ( expr->Tag() ) {
             case ::TYPE_DOUBLE:
                 Builder()->addInstruction(result, ::hilti::instruction::integer::AsUDouble, hexpr);
                 break;
    ...
    ok = ok || CompileOperator(expr, result, ::TYPE_INT, UNARY_OP_FUNC {
             switch ( expr->Tag() ) {
             case ::TYPE_DOUBLE:
                 Builder()->addInstruction(result, ::hilti::instruction::integer::AsSDouble, hexpr);
                 break;
    ...
    Error(::util::fmt("no support in ArithCoerceExpr for coercion from %s to %s",
                      ::type_name(expr->Op()->Type()->Tag()), ::type_name(expr->Type()->Tag())),
          expr);
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(const ::Expr* lhs,
                                                                      const ::Expr* rhs)
{
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::Expr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
    switch ( lhs->Tag() ) {
    case EXPR_NAME:
    ...
    case EXPR_REF:
        return CompileAssign(static_cast<const ::RefExpr*>(lhs), rhs);

    case EXPR_INDEX:
        return CompileAssign(static_cast<const ::IndexExpr*>(lhs), rhs);

    case EXPR_FIELD:
        return CompileAssign(static_cast<const ::FieldExpr*>(lhs), rhs);
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::RefExpr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
    return CompileAssign(lhs->Op(), rhs);
}

std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::NameExpr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
    auto id = lhs->Id();
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::IndexExpr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::FieldExpr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
...
std::shared_ptr<::hilti::Expression> ExpressionBuilder::CompileAssign(
    const ::ListExpr* lhs, std::shared_ptr<::hilti::Expression> rhs)
{
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::DivideExpr* expr)
{
    ...
    ok = ok ||
         CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT, ::hilti::instruction::integer::Div);
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                               ::hilti::instruction::integer::Div);
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                               ::hilti::instruction::double_::Div);
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::IndexExpr* expr)
{
    ...
    case TYPE_VECTOR: {
        ...
        Builder()->addInstruction(idx, ::hilti::instruction::tuple::Index, op2,
    ...
    case TYPE_STRING: {
        ...
        Builder()->addInstruction(offset, ::hilti::instruction::tuple::Index, op2,
                                  ::hilti::builder::integer::create(0));
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::ListExpr* expr)
{
    ...
    auto targets = HasTargetType() && TargetType()->Tag() != TYPE_ANY ?
                       TargetType()->AsTypeList()->Types() :
                       nullptr;
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::ModExpr* expr)
{
    ...
    ok = ok ||
         CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT, ::hilti::instruction::integer::Mod);
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                               ::hilti::instruction::integer::Mod);
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                               ::hilti::instruction::double_::Mod);
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::NegExpr* expr)
{
    ...
    ok = ok || CompileOperator(expr, result, ::TYPE_INT, UNARY_OP_FUNC {
             Builder()->addInstruction(result, ::hilti::instruction::integer::Sub,
                                       ::hilti::builder::integer::create(0), op);
         });
    ...
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, UNARY_OP_FUNC {
             Builder()->addInstruction(result, ::hilti::instruction::double_::Sub,
                                       ::hilti::builder::double_::create(0), op);
         });
...
shared_ptr<::hilti::Expression> ExpressionBuilder::CompileListExprToRecordTuple(
    const ::ListExpr* lexpr, const ::RecordType* rtype)
{
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::RecordCoerceExpr* expr)
{
    ...
    if ( expr->Op()->Tag() == EXPR_RECORD_CONSTRUCTOR ) {
        auto rctor = static_cast<const ::RecordConstructorExpr*>(expr->Op());
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::SetConstructorExpr* expr)
{
    ...
    if ( ttype->IsUnspecifiedTable() ) {
        auto tt = TargetType();
        auto rt = ast::rtti::checkedCast<::hilti::type::Reference>(HiltiType(tt));
        auto stype = ast::rtti::checkedCast<::hilti::type::Set>(rt->argType());
    ...
    loop_over_list(exprs, i) elems.push_back(HiltiExpression(exprs[i]));
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::SizeExpr* expr)
{
    ...
    ok = ok || CompileOperator(expr, result, ::TYPE_TABLE, UNARY_OP_FUNC {
             if ( expr->Op()->Type()->AsTableType()->IsSet() )
                 Builder()->addInstruction(result, ::hilti::instruction::set::Size, op);
             else
                 Builder()->addInstruction(result, ::hilti::instruction::map::Size, op);
         });

    ok = ok || CompileOperator(expr, result, ::TYPE_LIST, UNARY_OP_FUNC {
             int size = expr->Op()->Type()->AsTypeList()->Types()->length();

             Builder()->addInstruction(result, ::hilti::instruction::operator_::Assign,
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::SubExpr* expr)
{
    ...
    ok = ok ||
         CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT, ::hilti::instruction::integer::Sub);
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                               ::hilti::instruction::integer::Sub);
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                               ::hilti::instruction::double_::Sub);
    ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_INTERVAL,
                               ::hilti::instruction::time::SubInterval);
    ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_TIME,
                               ::hilti::instruction::time::SubTime);
    ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::TableCoerceExpr* expr)
{
    ...
    if ( tt->IsSet() ) {
        ...
        auto mtype = ast::rtti::checkedCast<::hilti::type::Map>(rt->argType());
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::TimesExpr* expr)
{
    ...
    ok = ok ||
         CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT, ::hilti::instruction::integer::Mul);
    ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                               ::hilti::instruction::integer::Mul);
    ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                               ::hilti::instruction::double_::Mul);
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::VectorCoerceExpr* expr)
{
    ...
    auto rt = ast::rtti::checkedCast<::hilti::type::Reference>(HiltiType(vt));
    auto vtype = ast::rtti::checkedCast<::hilti::type::Vector>(rt->argType());
...
shared_ptr<::hilti::Expression> ExpressionBuilder::Compile(const ::RelExpr* expr)
{
    ...
    switch ( expr->Tag() ) {
    case EXPR_LT: {
        ok = ok || CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT,
                                   ::hilti::instruction::integer::Slt);
        ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                                   ::hilti::instruction::integer::Ult);
        ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                                   ::hilti::instruction::double_::Lt);
        ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
                                   ::hilti::instruction::interval::Lt);
        ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_TIME,
                                   ::hilti::instruction::time::Lt);
        break;
    }

    case EXPR_LE: {
        ok = ok || CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT,
                                   ::hilti::instruction::integer::Sleq);
        ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                                   ::hilti::instruction::integer::Uleq);
        ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                                   ::hilti::instruction::double_::Leq);
        ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
                                   ::hilti::instruction::interval::Leq);
        ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_TIME,
                                   ::hilti::instruction::time::Leq);
        break;
    }

    case EXPR_GT: {
        ok = ok || CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT,
                                   ::hilti::instruction::integer::Sgt);
        ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                                   ::hilti::instruction::integer::Ugt);
        ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                                   ::hilti::instruction::double_::Gt);
        ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
                                   ::hilti::instruction::interval::Gt);
        ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_TIME,
                                   ::hilti::instruction::time::Gt);
        break;
    }

    case EXPR_GE: {
        ok = ok || CompileOperator(expr, result, ::TYPE_INT, ::TYPE_INT,
                                   ::hilti::instruction::integer::Sgeq);
        ok = ok || CompileOperator(expr, result, ::TYPE_COUNT, ::TYPE_COUNT,
                                   ::hilti::instruction::integer::Ugeq);
        ok = ok || CompileOperator(expr, result, ::TYPE_DOUBLE, ::TYPE_DOUBLE,
                                   ::hilti::instruction::double_::Geq);
        ok = ok || CompileOperator(expr, result, ::TYPE_INTERVAL, ::TYPE_INTERVAL,
                                   ::hilti::instruction::interval::Geq);
        ok = ok || CompileOperator(expr, result, ::TYPE_TIME, ::TYPE_TIME,
                                   ::hilti::instruction::time::Geq);
        break;
    }
