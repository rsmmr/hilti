...
StatementBuilder::StatementBuilder(class ModuleBuilder* mbuilder) : BuilderBase(mbuilder)
{
}
...
void StatementBuilder::Compile(const ::AddStmt* stmt)
{
    ...
    switch ( expr->Tag() ) {
    case EXPR_INDEX: {
        auto iexpr = static_cast<const IndexExpr*>(expr);
...
void StatementBuilder::Compile(const ::DelStmt* stmt)
{
    ...
    case EXPR_FIELD: {
        auto fexpr = static_cast<const FieldExpr*>(expr);
...
{
    auto ev = static_cast<const ::EventExpr*>(stmt->StmtExpr());
...
void StatementBuilder::CompileForStmtTable(const ::ForStmt* stmt)
{
    ...
    if ( ! type->IsSet() ) {
        auto rtype = ::ast::rtti::checkedCast<::hilti::type::Reference>(HiltiType(type));
        auto mtype = ::ast::rtti::checkedCast<::hilti::type::Map>(rtype->argType());
        auto key = Builder()->addTmp("key", mtype->keyType());
        Builder()->addInstruction(key, ::hilti::instruction::tuple::Index, iter,
    ...
    if ( vars->length() == 1 ) {
        ...
        loop_over_list(*vars, i)
        {
            auto id = (*vars)[i];
            auto dst = ::hilti::builder::id::create(HiltiSymbol(id));
            Builder()->addInstruction(dst, ::hilti::instruction::tuple::Index, iter,
    ...
    auto body_stmt = ::ast::rtti::checkedCast<statement::Block>(body->block());
...
void StatementBuilder::CompileForStmtVector(const ::ForStmt* stmt)
{
    ...
    Builder()->addInstruction(iter, ::hilti::instruction::operator_::Assign,
                              ::hilti::builder::integer::create(0));
...
void StatementBuilder::Compile(const ::PrintStmt* stmt)
{
    ...
    loop_over_list(exprs, i)
    {
        ...
        if ( i > 0 ) {
            ...
            auto t =
                ::hilti::builder::tuple::create({op, ::hilti::builder::boolean::create(false)});
            Builder()->addInstruction(::hilti::instruction::flow::CallVoid, print, t);
        }
        ...
        if ( i < (size - 1) ) {
            auto t =
                ::hilti::builder::tuple::create({hexpr, ::hilti::builder::boolean::create(false)});
            Builder()->addInstruction(::hilti::instruction::flow::CallVoid, print, t);
        }
