...
ValueBuilder::ValueBuilder(class ModuleBuilder* mbuilder) : BuilderBase(mbuilder)
{
}
...
shared_ptr<::hilti::Expression> ValueBuilder::DefaultInitValue(const ::BroType* type)
{
    ...
    auto rtype = ::ast::rtti::tryCast<::hilti::type::Reference>(htype);

    switch ( type->Tag() ) {
    case TYPE_TABLE: {
        if ( type->IsSet() ) {
            auto stype = ::ast::rtti::checkedCast<::hilti::type::Set>(rtype->argType());
            return ::hilti::builder::set::create(stype->argType(), {});
        }

        else {
            auto mtype = ::ast::rtti::checkedCast<::hilti::type::Map>(rtype->argType());
            return ::hilti::builder::map::create(mtype->keyType(), mtype->valueType(), {});
        }
    }

    case TYPE_VECTOR: {
        auto vtype = ::ast::rtti::checkedCast<::hilti::type::Vector>(rtype->argType());
    ...
    default:
        ...
        Error(::util::fmt("ValueBuilder does not define a initialization value for type %s",
                          ::type_name(type->Tag())),
              type);
    }
...
shared_ptr<::hilti::Expression> ValueBuilder::TypeVal(const ::BroType* type)
{
    ...
    auto args =
        ::hilti::builder::tuple::create({::hilti::builder::string::create(type->GetName())});
...
shared_ptr<hilti::Expression> ValueBuilder::Compile(const ::Val* val, const ::BroType* target_type,
                                                    bool init)
{
...
std::shared_ptr<::hilti::Expression> ValueBuilder::Compile(const ::TableVal* val)
{
    ...
    if ( ttype->IsSet() ) {
        ...
        auto rtype = ::ast::rtti::checkedCast<::hilti::type::Reference>(htype);
        auto stype = ::ast::rtti::checkedCast<::hilti::type::Set>(rtype->argType());
        ...
        if ( Attr* def_attr = val->FindAttr(ATTR_DEFAULT) ) {
            ...
            if ( dtype->Tag() != TYPE_FUNC )
                ...
                if ( m.first && m.second ) {
                    ...
                    def = ::hilti::builder::callable::create(mtype->valueType(), {mtype->keyType()},
                                                             hilti_func, {});
                }
