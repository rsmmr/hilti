...
static inline void debug_msg(file_analysis::Analyzer* analyzer, const char* msg, int len,
                             const u_char* data)
{
#ifdef DEBUG
    if ( data ) {
        PLUGIN_DBG_LOG(HiltiPlugin, "[%s/%" PRIu32 "/file] %s: |%s%s|",
                       file_mgr->GetComponentName(analyzer->Tag()).c_str(), analyzer->GetID(), msg,
                       fmt_bytes((const char*)data, min(40, len)), len > 40 ? "..." : "");
    }

    else {
        PLUGIN_DBG_LOG(HiltiPlugin, "[%s/%" PRIu32 "/file] %s",
                       file_mgr->GetComponentName(analyzer->Tag()).c_str(), analyzer->GetID(), msg);
    }
#endif
}

Spicy_FileAnalyzer::Spicy_FileAnalyzer(RecordVal* arg_args, file_analysis::File* arg_file)
    : file_analysis::Analyzer(arg_args, arg_file)
{
    args = arg_args;
    Ref(args);

    cookie.type = SpicyCookie::FILE;
    cookie.file_cookie.analyzer = this;
    cookie.file_cookie.tag = file_analysis::Tag(); // Error until we know it.
}

Spicy_FileAnalyzer::~Spicy_FileAnalyzer()
{
    Unref(args);
    cookie.file_cookie.analyzer = 0;
}

void Spicy_FileAnalyzer::Init()
{
...
int Spicy_FileAnalyzer::FeedChunk(int len, const u_char* chunk, bool eod)
{
    // TODO: This is very similar to SpicyAnalyzer.cc. Can we factor out common code?
...
bool Spicy_FileAnalyzer::DeliverStream(const u_char* data, uint64 len)
{
    ...
    if ( rc >= 0 ) {
        debug_msg(this, ::util::fmt("parsing %s, skipping further content",
...
bool Spicy_FileAnalyzer::Undelivered(uint64 offset, uint64 len)
{
    return file_analysis::Analyzer::Undelivered(offset, len);
}

bool Spicy_FileAnalyzer::EndOfFile()
{
...
void Spicy_FileAnalyzer::ParseError(const string& msg)
{
