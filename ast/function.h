...
public:
    ...
    Function(shared_ptr<ID> id, shared_ptr<FunctionType> ftype, shared_ptr<Module> module,
    ...
    shared_ptr<Statement> body() const
    {
        return _body;
    }
    ...
    {
        return util::fmt("%s (%s)", _id ? _id->name().c_str() : "<no id>",
                         _ftype ? _ftype->render().c_str() : "<no type>");
    }
