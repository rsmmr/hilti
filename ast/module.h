...
public:
    ...
    shared_ptr<BodyStatement> body() const
    {
        return _body;
    }
    ...
    {
        for ( auto i : _imported ) {
            if ( i->pathAsString() == id->pathAsString() )
                return;
        }
    ...
    const id_list& importedIDs() const
    {
        return _imported;
    }
    ...
    const type_list& exportedTypes() const
    {
        return _exported_types;
    }
