...
public:
    ...
    /// separator
    template <typename T>
    void printList(const std::list<T>& elems, const string& separator,
                   const string& elem_prefix = "", const string& elem_postfix = "")
    {
        Printer& p = *this;
        bool first = true;
        for ( auto e : elems ) {
    ...
    template <typename T>
    void printList(const std::list<T>& elems, const string& separator, const string& elem_prefix,
                   const string& elem_postfix, std::function<string(T&)> fmt)
    {
        Printer& p = *this;
        bool first = true;
        for ( auto e : elems ) {
    ...
    /// XXX
    void setPrintOriginalIDs(bool org)
    {
        _orginal_ids = org;
    }

    /// XXX
    bool printOriginalIDs() const
    {
        return _orginal_ids;
    }

    /// XXX
    bool qualifyTypeIDs() const
    {
        return _qualify_type_ids;
    }

    /// XXX
    bool setQualifyTypeIDs(bool qualify)
    {
        return _qualify_type_ids = qualify;
    }
...
private:
    ...
    friend Printer<AI>& operator<<(Printer<AI>& p, Printer<AI>& (*pf)(Printer<AI>& p));
    template <typename AI>
    friend Printer<AI>& operator<<(Printer<AI>& p, std::ostream& (*pf)(std::ostream&));
    template <typename AI>
    friend Printer<AI>& operator<<(Printer<AI>& p, std::ios& (*pf)(std::ios&));
    template <typename AI>
    friend Printer<AI>& operator<<(Printer<AI>& p, std::ios_base& (*pf)(std::ios_base&));
    template <typename AI, typename T>
    friend Printer<AI>& operator<<(Printer<AI>& p, T t);
    template <typename AI, typename T>
    friend Printer<AI>& operator<<(Printer<AI>& p, shared_ptr<T> node);
    template <typename AI, typename T>
    friend Printer<AI>& operator<<(Printer<AI>& p, node_ptr<T> node);
...
template <typename AstInfo>
inline Printer<AstInfo>& operator<<(Printer<AstInfo>& p,
                                    Printer<AstInfo>& (*pf)(Printer<AstInfo>& p))
{
    p._prepLine();
    return (*pf)(p);
}
