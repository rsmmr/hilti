...
void hlt_struct_dtor(hlt_type_info* type, void* obj, hlt_execution_context* ctx)
{
    ...
    uint32_t mask = *((uint32_t*)((char*)obj + sizeof(__hlt_gchdr)));
    ...
    for ( int i = 0; i < type->num_params; i++ ) {
        ...
        if ( is_set )
            GC_DTOR_GENERIC((char*)obj + array[i].offset, types[i], ctx);
    }
}

// Generic version working with all struct types.
void* hlt_struct_clone_alloc(const hlt_type_info* ti, void* srcp, __hlt_clone_state* cstate,
                             hlt_exception** excpt, hlt_execution_context* ctx)
{
    return GC_NEW_CUSTOM_SIZE_GENERIC_REF(ti, ti->object_size, ctx);
}

void hlt_struct_clone_init(void* dstp, const hlt_type_info* ti, void* srcp,
    ...
    *((uint32_t*)(dst + sizeof(__hlt_gchdr))) = mask;
    ...
    for ( int i = 0; i < ti->num_params; i++ ) {
        ...
        if ( is_set ) {
            ...
            __hlt_clone((char*)dst + offset, types[i], (char*)src + offset, cstate, excpt, ctx);
...
hlt_string hlt_struct_to_string(const hlt_type_info* type, void* obj, int32_t options,
                                ...
                                hlt_execution_context* ctx)
{
    ...
    for ( int i = 0; i < type->num_params; i++ ) {
        if ( array[i].field[0] && array[i].field[1] && array[i].field[0] == '_' &&
             array[i].field[1] == '_' )
        ...
        if ( ! is_set )
            ...
            t = __hlt_object_to_string(types[i], (char*)obj + array[i].offset, options, seen, excpt,
                                       ctx);
...
hlt_hash hlt_struct_hash(const hlt_type_info* type, const void* obj, hlt_exception** excpt,
                         hlt_execution_context* ctx)
{
    hlt_hash hash = 0;
    ...
    for ( int i = 0; i < type->num_params; i++ ) {
        ...
        if ( is_set ) {
            assert(types[i]->hash);
            hlt_hash h = (types[i]->hash)(types[i], (char*)obj + array[i].offset, excpt, ctx);
            hash += h;
            hash *= 2654435761;
        }
        else
...
int8_t hlt_struct_equal(const hlt_type_info* type1, const void* obj1, const hlt_type_info* type2,
                        const void* obj2, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    uint32_t mask1 = *((uint32_t*)((char*)obj1 + sizeof(__hlt_gchdr)));
    uint32_t mask2 = *((uint32_t*)((char*)obj2 + sizeof(__hlt_gchdr)));
    ...
    for ( int i = 0; i < type1->num_params; i++ ) {
        ...
        if ( is_set ) {
            ...
            if ( ! (types1[i]->equal)(types1[i], (char*)obj1 + array1[i].offset, types2[i],
                                      (char*)obj2 + array2[i].offset, excpt, ctx) )
                return 0;
        }
    }
...
int8_t hlt_struct_is_set(const hlt_type_info* type, void* obj, int index, hlt_exception** excpt,
                         hlt_execution_context* ctx)
{
    if ( type->type != HLT_TYPE_STRUCT ) {
        hlt_set_exception(excpt, &hlt_exception_type_error, 0, ctx);
...
void* hlt_struct_get(const hlt_type_info* type, void* obj, int index, hlt_exception** excpt,
                     hlt_execution_context* ctx)
{
    if ( type->type != HLT_TYPE_STRUCT ) {
        hlt_set_exception(excpt, &hlt_exception_type_error, 0, ctx);
    ...
    struct field* array = (struct field*)type->aux;
    return (char*)obj + array[index].offset;
}

hlt_struct_field hlt_struct_get_type(const hlt_type_info* type, int index, hlt_exception** excpt,
                                     hlt_execution_context* ctx)
{
