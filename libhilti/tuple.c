...
void hlt_tuple_dtor(hlt_type_info* ti, void* obj, hlt_execution_context* ctx)
{
    ...
    for ( int i = 0; i < ti->num_params; i++ ) {
        __hlt_object_dtor(types[i], (char*)obj + elements[i].offset, "tuple-dtor", ctx);
    }
}
...
void hlt_tuple_cctor(hlt_type_info* ti, void* obj, hlt_execution_context* ctx)
{
    ...
    for ( int i = 0; i < ti->num_params; i++ ) {
        __hlt_object_cctor(types[i], (char*)obj + elements[i].offset, "tuple-dtor", ctx);
    }
}

// Generic version working with all tuple types.
void hlt_tuple_clone_init(void* dstp, const hlt_type_info* ti, void* srcp,
...
hlt_string hlt_tuple_to_string(const hlt_type_info* type, void* obj, int32_t options,
                               ...
                               hlt_execution_context* ctx)
{
    ...
    for ( i = 0; i < type->num_params; i++ ) {
        ...
        hlt_string t =
            __hlt_object_to_string(types[i], (char*)obj + elements[i].offset, 0, seen, excpt, ctx);
                        ...
                        hlt_execution_context* ctx)
{
    ...
    for ( int i = 0; i < type->num_params; i++ ) {
        assert(types[i]->hash);
        hash += (*types[i]->hash)(types[i], (char*)obj + elements[i].offset, excpt, ctx);
        hash *= 2147483647;
    }
...
int8_t hlt_tuple_equal(const hlt_type_info* type1, const void* obj1, const hlt_type_info* type2,
                       const void* obj2, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    for ( int i = 0; i < type1->num_params; i++ ) {
        ...
        if ( ! (*types[i]->equal)(types[i], (char*)obj1 + elements[i].offset, types[i],
                                  (char*)obj2 + elements[i].offset, excpt, ctx) )
            return 0;
    }
...
void* hlt_tuple_get(const hlt_type_info* type, void* obj, int index, hlt_exception** excpt,
                    hlt_execution_context* ctx)
{
    if ( type->type != HLT_TYPE_TUPLE ) {
        hlt_set_exception(excpt, &hlt_exception_type_error, 0, ctx);
    ...
    struct _element* elements = (struct _element*)type->aux;
    return (char*)obj + elements[index].offset;
}

hlt_tuple_element hlt_tuple_get_type(const hlt_type_info* type, int index, hlt_exception** excpt,
                                     hlt_execution_context* ctx)
{
    hlt_tuple_element e = {0, 0};
