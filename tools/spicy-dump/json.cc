...
static void json_dump_unit(const hlt_type_info* type, void* obj, int indent, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    spicy_unit_cookie cookie = 0;
    spicy_unit_item item;
    ...
    while ( (cookie = spicy_unit_iterate(&item, type, unit, 0, cookie, excpt, ctx)) ) {
...
void json_dump_object(const hlt_type_info* type, void* obj, int indent, hlt_exception** excpt, hlt_execution_context* ctx)
{
    ...
    switch ( id ) {
     ...
     case SPICY_TYPE_SINK:
        json_dump_sink(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_STRING:
        json_dump_string(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_TIME:
        json_dump_time(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_TUPLE:
        json_dump_tuple(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_UNIT:
        json_dump_unit(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_VECTOR:
        json_dump_vector(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_VOID:
        json_dump_void(type, obj, indent, excpt, ctx);
        break;

     case SPICY_TYPE_NONE:
        fprintf(stderr, "internal error: Spicy type not set in HILTI rtti object when rendering (HILTI type: %d/%s)\n", type->type, type->tag);
        abort();

     default:
        fprintf(stderr, "internal error: Spicy type %" PRIu64 " not supported fo rendering (HILTI type: %d/%s)\n", id, type->type, type->tag);
        abort();
    }
}
