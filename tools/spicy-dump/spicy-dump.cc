...
spicy::CompilerContext* PacContext = nullptr;
...
void parseSingleInput(spicy_parser* p)
{
    hlt_execution_context* ctx = hlt_global_execution_context();
    hlt_exception* excpt = 0;
...
bool jitSpicy(const std::list<string>& spcy, std::shared_ptr<spicy::Options> options)
{
    ...
    PacContext = new spicy::CompilerContext(options);
    ...
    if ( ! JitParsers ) {
        fprintf(stderr, "jitSpicy error: no function 'spicy_parsers'");
        return false;
    }
