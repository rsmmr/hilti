...
//
// This code is quite messy, needs a cleanup once we know what functionality
// will still be needed.
//
// ALso, this is mostly written in C, except for the JIT usage which requires
// the C++ interface. Maybe we should write the whole driver in C++?
...
binpac::CompilerContext::binpac_parsers_func jit_binpac_parsers = nullptr;
shared_ptr<binpac::CompilerContext> ctx = nullptr;
...
void listParsers()
{
    ...
    assert(jit_binpac_parsers);

    hlt_exception* excpt = 0;
    hlt_execution_context* ctx = hlt_global_execution_context();

    hlt_list* parsers = (*jit_binpac_parsers)(&excpt, ctx);
...
{
    ctx = std::make_shared<binpac::CompilerContext>();
    ctx->enableDebug(cgdbg);

    std::list<shared_ptr<hilti::Module>> hilti_modules;

    for ( auto p : pac2 ) {
        auto module = ctx->load(p, true);

        if ( ! module )
            return false;

        auto hltmod = ctx->compile(module, debug);
    ...
    auto llvm_module = ctx->linkModules("<jit analyzers>", hilti_modules);

    if ( llvm_module )
        return false;

    jit_binpac_parsers = ctx->jitBinpacParser(llvm_module, optlevel);

    return (jit_binpac_parsers != 0);
}
...
Flow* bulkFeedPiece(binpac_parser* parser, Flow* flow, int eof, const char* data, int size, const char* fid, const char* t)
{
    ...
    int8_t* tmp = (int8_t*) hlt_malloc(size);
    assert(tmp);
    ...
    if ( ! flow ) {
        flow = (Flow*) hlt_malloc(sizeof(Flow));
        flow->input = input;
        flow->resume = 0;
        flow->stopped = 0;
        result = flow;
