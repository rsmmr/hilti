...
bool Coercer::_coerceTo(shared_ptr<Expression> expr, shared_ptr<Type> src, shared_ptr<Type> dst,
                        shared_ptr<Expression>* result, string* error)
{
    ...
    if ( matches.size() > 1 ) {
        if ( error )
            *error =
                util::fmt("coerce from type %s to %s is ambigious", src->render(), dst->render());
    ...
    if ( result ) {
        ...
        *result =
            OperatorRegistry::globalRegistry()->resolveOperator(op, ops, module, expr->location());
    }

#if 0
    if ( result ) {
        if ( expr )
            std::cerr << "  => " << (*result)->render() << " / " << typeName(*(*result).get()) << std::endl;
    }
#endif
