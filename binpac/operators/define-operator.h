...
#define opBeginH(OP) \
    class OP : public binpac::Operatpr {
    public:
       OP();
       virtual ~OP();
    protected:
       ...
       string                 __doc() const { return "<No documentation>"; }
...
protected:
    shared_ptr<Type> __typeOp1() const    { return ; }
    shared_ptr<Type> __typeOp2() const    { return std::make_shared<type::Integer>(); }
...
opBegin(Plus)
    op1(std::make_shared<type::Integer>())
    op2(std::make_shared<type::Integer>())
