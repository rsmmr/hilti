    ...
    : Field(id, std::make_shared<type::Bytes>(), cond, hooks, attrs, expression_list(), sinks, l)
{
    ...
    if ( ! transient() ) {
        ...
        auto hook_push = std::make_shared<binpac::Hook>(body_push, 254, false, true, parameter_list(), l);
        addHook(hook_push);
    }
    ...
    if ( until || until_including || while_ ) {
        ...
        auto hook = std::make_shared<binpac::Hook>(body, prio, false, true, parameter_list(), l);
        addHook(hook);
