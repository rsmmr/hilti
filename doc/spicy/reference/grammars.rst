...
.. _spicy_global_global:

``global``
    TODO.

.. _spicy_global_constants:

``const``
    TODO.

.. _spicy_global_import:
...
Unit Hooks
    ...
    ``%debug``
        ...
        *and* debugging is activated (i.e., has been compiled in and
        is enabled at run-time, see :ref:`spicy_debugging`).
...
Field and Variable Hooks
   ...
   :ref:`spicy_unit_variables` , like in this toy example::
...
Item Hooks
    ...
    :spicy:type:`list`, :spicy:type:`vector`) and execute each time one
    ...
    While this is parsing :spicy:type:`list` elements, each will be
...
across units: if you :ref:`import <spicy_global_import>` the
