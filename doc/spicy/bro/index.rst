...
analyzer as standard ``*.spicy`` code; and defining the analyzer's
...
Here's a simple grammar ``ssh.spicy`` to parse an SSH banner (e.g.,
``SSH-2.0-OpenSSH_6.1``):

.. literalinclude:: /../bro/spicy/ssh.spicy

This is indeed just a standard Spicy grammar, just as it can be
used with :ref:`spicy_spicy-driver`. It could include further hooks and
...
``grammar`` specification telling Bro which ``*.spicy`` file to use; (2)
...
.. literalinclude:: /../bro/spicy/ssh.evt

The ``grammar`` line specifies a ``*.spicy`` file to load. The path is
...
at ``hilti2/bro/spicy/``, and in any directories specified through the
environment variable ``BRO_SPICY_PATH`` (using the standard syntax of
colon-separated paths). The ``grammar`` line is optional, one can
leave it out if the ``*.spicy`` grammars get loaded otherwise (e.g.,
via the command line; see below). One can also specify multiple
``grammar`` entries.

.. note::

    Technically, the Bro plugin searches the ``*.spicy`` files inside
    the *build* directory at ``<build>/spicy`` because that will later
    ...
    back to the source directory ``hilti2/bro/spicy/`` so that one can
    edit files there and they will be pulled in directly.

The ``analyzer`` block starts with giving the new analyzer a name
(``spicy::SSH``). The namespace isn't mandatory, but we use it to
...
``analyzer::ANALYZER_SPICY_SSH``. (As you can see there's a bit of name
...
<name>(<parameters>)``. Let's break it down:
    ...
    - ``<hook>`` defines when an event should be triggered, and it
      ...
      <spicy_hooks>`, i.e., you give the fully-qualified path to the
    ...
    - The ``<parameters>`` define the event's parameters, specifying
      ...
      :ref:`spicy_bro-type-mapping` for the details). At runtime, when
...
.. literalinclude:: /../bro/tests/spicy/ssh-banner.bro
    :lines: 6-9
...
described above for ``*.spicy`` files---in ``hilti2/bro/spicy/`` and any
...
provides our analyzer ``spicy_SSH``, which generates one event
``ssh_banner``.
...
.. literalinclude:: /../bro/spicy/gzip.evt
...
.. literalinclude:: /../bro/spicy/gzip.spicy
    :lines: 6-32

For testing let's also provide an event handler in ``gzip-header.bro``:

.. literalinclude:: /../bro/tests/spicy/gzip-header.bro
    :lines: 6-9
...
``debug: bool`` (default: false)
    ...
    that :ref:`spicy_debugging` will be available (including the
...
``use_cache: bool`` (default: true)
    ...
    always pick up on changes made to ``*.spicy`` or ``*.evt`` files
