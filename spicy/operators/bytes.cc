...
opBegin(bytes::Match : MethodCall)
    ...
    opDoc(
        "Matches the ``bytes`` object against the regular expression *r*. Returns the matching "
        "part, or if *n* is given the corresponding subgroup within *r*.");
...
opBegin(bytes::Strip : MethodCall)
    ...
    opDoc(
        "Strips off leading and/or trailing characters, as indicated by *side* with either if not "
        "given. By default it strips off all whitespace; alternatively any characters contained in "
        "*chars*.");
...
opBegin(bytes::ToUInt : MethodCall)
    ...
    opCallArg1("base",
               std::make_shared<type::OptionalArgument>(type::Integer::unsignedInteger(64)));

    opDoc(
        "Interprets the ``bytes`` as representing an ASCII-encoded number and converts it into an "
        "unsigned integer, using a base of *base*. If *base* is not given, the default is 10.");
...
opBegin(bytes::ToUIntBinary : MethodCall)
    ...
    opDoc(
        "Interprets the ``bytes`` as representing an binary number encoded with the given byte "
        "order, and converts it into an unsigned integer.");
...
opBegin(bytes::ToInt : MethodCall)
    ...
    opDoc(
        "Interprets the ``bytes`` as representing an ASCII-encoded number and converts it into a "
        "signed integer, using a base of *base*. If *base* is not given, the default is 10.");
...
opBegin(bytes::ToIntBinary : MethodCall)
    ...
    opDoc(
        "Interprets the ``bytes`` as representing an binary number encoded with the given byte "
        "order, and converts it into a signed integer.");
...
opBegin(bytes::ToTime : MethodCall)
    ...
    opDoc(
        "Interprets the ``bytes`` as representing a number of seconds since the epoch in the form "
        "of an ASCII-encoded number and converts it into a time value, using a base of *base*. If "
...
opBegin(bytes::ToTimeBinary : MethodCall)
    ...
    opDoc(
        "Interprets the ``bytes`` as representing as number of seconds since the epoch in the form "
        "of an binary number encoded with the given byte order, and converts it into a time "
        "value.");
...
opBegin(bytes::Split1 : MethodCall)
    ...
    opDoc(
        "Splits at the first occurence of ``sep``, returning a pair of ``bytes`` representing "
        "everything before and afterwards, respectively. If *sep* is skipped, the default is to "
        "split at any sequence of white-space.");
...
opBegin(bytes::Split : MethodCall)
    ...
    opDoc(
        "Splits at each occurence of ``sep``, returning a vector of ``bytes`` representing each "
        "piece excluding the separators. If *sep* is skipped, the default is to split at any "
...
opBegin(bytes::Join : MethodCall)
    ...
    opDoc(
        "Renders the elements of *l* into textual form and joins them into a single bytes object "
        "using the given one as separator.");
