...
using namespace spicy::statement;
...
ForEach::ForEach(shared_ptr<ID> id, shared_ptr<spicy::Expression> seq, shared_ptr<Block> body,
                 const Location& l)
    : Statement(l)
{
    _id = id;
...
shared_ptr<spicy::Expression> ForEach::sequence() const
{
    return _seq;
}
...
statement::Expression::Expression(shared_ptr<spicy::Expression> expr, const Location& l)
    : Statement(l)
{
    _expr = expr;
    addChild(_expr);
...
shared_ptr<spicy::Expression> statement::Expression::expression() const
{
    return _expr;
}

IfElse::IfElse(shared_ptr<spicy::Expression> cond, shared_ptr<Statement> true_,
...
shared_ptr<spicy::Expression> IfElse::condition() const
{
    return _cond;
}
...
Return::Return(shared_ptr<spicy::Expression> expr, const Location& l) : Statement(l)
{
    _expr = expr;
    addChild(_expr);
}

shared_ptr<spicy::Expression> Return::expression() const
{
    return _expr;
}
