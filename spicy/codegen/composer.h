...
#ifndef SPICY_CODEGEN_COMPOSER_H
#define SPICY_CODEGEN_COMPOSER_H
...
private:
    ...
    void _hiltiCallComposeFunction(shared_ptr<spicy::type::Unit> unit,
                                   shared_ptr<hilti::Expression> func);

    // Run a field's hooks.
    void _hiltiRunFieldHooks(shared_ptr<type::unit::item::Field> field);

    // Prints the given message to the spicy-compose debug stream.
    void _hiltiDebug(const string& msg);

    // Prints the given message to the spicy-compose-verbose debug stream.
