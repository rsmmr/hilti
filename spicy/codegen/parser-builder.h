...
#ifndef SPICY_CODEGEN_PARSER_BUILDER_H
#define SPICY_CODEGEN_PARSER_BUILDER_H
...
private:
    ...
    shared_ptr<hilti::Expression> _hiltiCallParseFunction(
        shared_ptr<spicy::type::Unit> unit, shared_ptr<hilti::Expression> func,
        bool catch_parse_error, shared_ptr<hilti::Type> presult_value_type);

    // Prints the given message to the 'spicy' debug stream.
    void _hiltiDebug(const string& msg);

    // Prints the given message to the 'spicy-verbose' debug stream.
    void _hiltiDebugVerbose(const string& msg);

    // Prints the given token to spicy-verbose.
    void _hiltiDebugShowToken(const string& tag, shared_ptr<hilti::Expression> token);

    // Prints the upcoming input bytes to spicy-verbose.
