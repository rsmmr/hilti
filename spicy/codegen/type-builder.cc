...
void TypeBuilder::_addHostType(shared_ptr<::hilti::Type> type, int spicy_type,
                               shared_ptr<::hilti::Expression> aux)
{
    auto i = ::hilti::builder::integer::create(spicy_type);
...
shared_ptr<::hilti::Type> TypeBuilder::_buildType(shared_ptr<::hilti::Type> type, int spicy_type,
                                                  shared_ptr<::hilti::Expression> aux)
{
#if 0
    ...
    _addHostType(type, spicy_type, aux);
    return type;
...
shared_ptr<hilti::Expression> TypeBuilder::hiltiAddParseObjectTypeInfo(shared_ptr<Type> unit)
{
    ...
    // unit.
    //
    // Note that when changing anything here, libspicy/rtti.c must be
    // adapted accordingly.
    ...
    for ( auto i : u->flattenedItems() ) {
        ...
        spicy_unit_item_kind kind = SPICY_UNIT_ITEM_NONE;

        if ( ast::rtti::isA<type::unit::item::Field>(i) )
            kind = SPICY_UNIT_ITEM_FIELD;

        else if ( ast::rtti::isA<type::unit::item::Variable>(i) )
            kind = SPICY_UNIT_ITEM_VARIABLE;
...
void TypeBuilder::visit(type::Bitfield* b)
{
    ...
    auto t = _buildType(tt, SPICY_TYPE_BITFIELD);
...
void TypeBuilder::visit(type::Enum* e)
{
    ...
    ti.hilti_type = _buildType(et, SPICY_TYPE_ENUM);
    ti.hilti_default =
...
void TypeBuilder::visit(type::EmbeddedObject* o)
{
    ...
    TypeInfo ti;
    ti.hilti_type = _buildType(etype, SPICY_TYPE_EMBEDDED_OBJECT);
    setResult(ti);
}
...
void TypeBuilder::visit(type::Integer* i)
{
    ...
    TypeInfo ti;
    ti.hilti_type = _buildType(tag, hilti::builder::integer::type(i->width()), SPICY_TYPE_INTEGER, signed_);
    setResult(ti);
#else
    auto signed_ = i->signed_() ? SPICY_TYPE_INTEGER_SIGNED : SPICY_TYPE_INTEGER_UNSIGNED;
...
void TypeBuilder::visit(type::Optional* o)
{
    ...
    TypeInfo ti;
    ti.hilti_type = _buildType(ht, SPICY_TYPE_OPTIONAL);
    setResult(ti);
}
...
{
    auto t = hilti::builder::type::byName("SpicyHilti::Sink");
