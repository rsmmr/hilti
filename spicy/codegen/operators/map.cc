...
#include <spicy/autogen/operators/map.h>
...
void CodeBuilder::visit(ctor::Map* m)
{
    ...
    auto result = hilti::builder::map::create(ktype, vtype, elems, nullptr, ::hilti::AttributeSet(),
                                              m->location());
    setResult(result);
}
...
void CodeBuilder::visit(expression::operator_::map::Get* i)
{
    ...
    if ( def ) {
        ...
        cg()->builder()->addInstruction(result, hilti::instruction::map::GetDefault, op1, idx,
                                        hdef);
    }
