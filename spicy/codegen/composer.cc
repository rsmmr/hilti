...
class spicy::codegen::ComposerState {
public:
    ComposerState(shared_ptr<spicy::type::Unit> unit, shared_ptr<hilti::Expression> self = nullptr,
...
ComposerState::ComposerState(shared_ptr<spicy::type::Unit> arg_unit,
...
shared_ptr<spicy::type::Unit> Composer::unit() const
{
    return state()->unit;
}
...
void Composer::_hiltiCallComposeFunction(shared_ptr<spicy::type::Unit> unit,
                                  ...
                                  shared_ptr<type::unit::item::Field> field)
{
    ...
    if ( cg()->options().debug > 0 && ! ast::rtti::isA<type::Unit>(field->type()) )
        cg()->builder()->addDebugMsg("spicy-compose", util::fmt("%s = %%s", field->id()->name()),
                                     data);
...
{
    if ( cg()->options().debug > 0 )
        cg()->builder()->addDebugMsg("spicy-compose", msg);
}

void Composer::_hiltiDebugVerbose(const string& msg)
{
    if ( cg()->options().debug > 0 )
        cg()->builder()->addDebugMsg("spicy-compose-verbose", string("- ") + msg);
}
...
void Composer::_hiltiComposeError(const string& msg)
{
    ...
    auto etype = builder::type::byName("SpicyHilti::ComposeError");
    auto excpt =
