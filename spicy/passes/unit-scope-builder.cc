...
UnitScopeBuilder::UnitScopeBuilder() : Pass<AstInfo>("spicy::UnitScopeBuilder", false)
{
}
...
void UnitScopeBuilder::visit(declaration::Type* t)
{
    ...
    uscope->insert(std::make_shared<ID>("self"),
                   std::make_shared<expression::ParserState>(expression::ParserState::SELF, nullptr,
                                                             unit, unit));
    uscope->setParent(current<Module>()->body()->scope());

    for ( auto p : unit->parameters() )
        uscope->insert(p->id(),
                       std::make_shared<expression::ParserState>(expression::ParserState::PARAMETER,
                                                                 p->id(), unit));
    ...
    for ( auto i : unit->flattenedItems() ) {
        ...
        iscope->insert(std::make_shared<ID>("$$"),
                       std::make_shared<
                           expression::ParserState>(expression::ParserState::DOLLARDOLLAR, nullptr,
                                                    unit, dd));
        ...
        for ( auto h : i->hooks() ) {
            ...
            if ( h->foreach () ) {
                ...
                h->body()->scope()->insert(
                    std::make_shared<ID>("$$"),
                    std::make_shared<expression::ParserState>(expression::ParserState::DOLLARDOLLAR,
                                                              nullptr, unit, dd));
            }
        }
    }
