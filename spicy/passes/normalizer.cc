...
#include <spicy/autogen/operators/enum.h>
#include <spicy/autogen/operators/unit.h>

using namespace spicy;
using namespace spicy::passes;

Normalizer::Normalizer() : ast::Pass<AstInfo>("spicy::Normalizer", true)
{
}
            ...
            if ( auto call =
                ...
                if ( pstate && pstate->kind() == expression::ParserState::DOLLARDOLLAR ) {
                    ...
                    auto nattr = std::make_shared<
                        spicy::expression::UnresolvedOperator>(operator_::Attribute, ops);
...
void Normalizer::visit(spicy::expression::operator_::unit::SetPosition* i)
{
    auto unit = ast::rtti::checkedCast<type::Unit>(i->op1()->type());
    unit->enableBuffering();
}

void Normalizer::visit(spicy::expression::operator_::unit::Offset* i)
{
    auto unit = ast::rtti::checkedCast<type::Unit>(i->op1()->type());
    unit->enableBuffering();
}

void Normalizer::visit(spicy::expression::operator_::unit::Input* i)
{
    auto unit = ast::rtti::checkedCast<type::Unit>(i->op1()->type());
    unit->enableBuffering();
}
...
void Normalizer::visit(spicy::expression::operator_::unit::TryAttribute* i)
{
    i->setUsesTryAttribute(true);
}

void Normalizer::visit(spicy::Expression* i)
{
