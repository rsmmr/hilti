...
hlt_string hlt_bool_to_string(const hlt_type_info* type, const void* obj, int32_t options, hlt_exception* exception)
{
    assert(type->type == HLT_TYPE_BOOL);
    return *((int8_t*)obj) ? &True : &False;
}

int64_t hlt_bool_to_int64(const hlt_type_info* type, const void* obj, hlt_exception* expt)
{
    assert(type->type == HLT_TYPE_BOOL);
