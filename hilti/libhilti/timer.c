...
extern void hlt_closure_execute(hlt_continuation* closure, hlt_exception** excpt);

static void __hlt_timer_fire(hlt_timer* timer, hlt_exception** excpt)
{
    switch (timer->type) {
      case HLT_TIMER_CLOSURE:
        hlt_closure_execute(timer->cookie.closure, excpt);
        break;
...
hlt_timer* __hlt_timer_new_closure(hlt_continuation* closure, hlt_exception** excpt)
{
    ...
    timer->type = HLT_TIMER_CLOSURE;
    timer->cookie.closure = closure;
    return timer;
}
