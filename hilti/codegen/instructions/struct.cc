...
static string _opToStr(shared_ptr<Expression> op)
{
    ...
    auto cexpr = ast::rtti::checkedCast<expression::Constant>(op);
    ;
...
{
    auto stype = ast::rtti::tryCast<type::Struct>(typedType(i->op1()));
...
void StatementBuilder::visit(statement::instruction::struct_::Get* i)
{
    ...
    auto result =
        cg()->llvmStructGet(stype, op1, _opToStr(i->op2()), nullptr, nullptr, i->location());
    cg()->llvmStore(i, result);
}
...
void StatementBuilder::visit(statement::instruction::struct_::GetDefault* i)
{
    ...
    auto stype = referencedType(i->op1());
    auto field = ast::rtti::checkedCast<type::Struct>(stype)->lookup(std::make_shared<ID>(fname));
    assert(field);

    auto op1 = cg()->llvmValue(i->op1());
    auto op3 = cg()->llvmValue(i->op3(), field->type());
    auto result = cg()->llvmStructGet(
        stype, op1, fname,
#if 1
...
void StatementBuilder::visit(statement::instruction::struct_::Set* i)
{
    auto stype = referencedType(i->op1());
    auto fname = _opToStr(i->op2());
    auto field = ast::rtti::checkedCast<type::Struct>(stype)->lookup(fname);
    assert(field);
