    ...
    /*
        def _codegen(self, cg):
            if isinstance(self.op2().type(), type.String):
                ...
                op3 = self.op3() if self.op3() else operand.LLVM(bytes.llvmEnd(cg),
       type.IteratorBytes())
    ...
    /*
        def _codegen(self, cg):
            if isinstance(self.op2().type(), type.String):
                ...
                # Bytes version.
                result = cg.llvmCallC("hlt::regexp_bytes_groups", [self.op1(), self.op2(),
       self.op3()])
    ...
    /*
        def _codegen(self, cg):
            if isinstance(self.op2().type(), type.String):
                ...
                op3 = self.op3() if self.op3() else operand.LLVM(bytes.llvmEnd(cg),
       type.IteratorBytes())
                result = cg.llvmCallC("hlt::regexp_bytes_match_token", [self.op1(), self.op2(),
       op3])
    ...
    /*
        def _codegen(self, cg):
            if isinstance(self.op2().type(), type.String):
                ...
                op3 = self.op3() if self.op3() else operand.LLVM(bytes.llvmEnd(cg),
       type.IteratorBytes())
                result = cg.llvmCallC("hlt::regexp_bytes_match_token_advance", [self.op1(),
       self.op2(), op3])
    ...
    /*
        def _codegen(self, cg):
            if isinstance(self.op2().type(), type.String):
                ...
                # Bytes version.
                result = cg.llvmCallC("hlt::regexp_bytes_span", [self.op1(), self.op2(),
       self.op3()])
