    ...
    */
    ...
    /*
        def _codegen(self, cg):
            ...
            result_func = cg.llvmNewHILTIFunction(cg.currentFunction(), result_name, [("__result",
       result_type)])
            result_block = result_func.append_basic_block("")

            cg.pushBuilder(result_block)
            cg.llvmStoreLocalVar(self.target().value(), result_func.args[3],
       frame=result_func.args[0])
