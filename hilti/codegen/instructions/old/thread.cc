    ...
    /*
        def _codegen(self, cg):
            ...
            if self.op3():
                ...
                vid = cg.llvmOp(vid)
                cg.llvmCallCInternal("__hlt_thread_mgr_schedule", [mgr, vid, cont,
       cg.llvmFrameExceptionAddr(), ctx])
                ...
                tcontext = _promoteContext(cg, tcontext, src_scope, src_context, dst_scope,
       dst_context)
                ...
                cg.llvmCallCInternal("__hlt_thread_mgr_schedule_tcontext", [mgr, ti, tcontext, cont,
       cg.llvmFrameExceptionAddr(), ctx])
                cg.llvmExceptionTest()
