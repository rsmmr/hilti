...
public:
   ...
   /// coerce_to: If given, the expr is first coerced into this type before
   /// it's evaluated. It's assumed that the coercion is legal and supported.
   void llvmValueInto(llvm::Value* dst, shared_ptr<Expression> expr, bool cctor, shared_ptr<hilti::Type> coerce_to = nullptr);
...
private:
   ...
   llvm::Value* normResult(const _LoadResult& result, shared_ptr<Type> type, bool cctor, llvm::Value* dst);
