...
iBegin(operator_::Begin, ".op.begin")
    ...
    iDoc(R"(
        Returns an iterator pointing to the first element of an iterable
        sequence.
    )")
iEnd
...
iBegin(operator_::End, ".op.end")
    ...
    iDoc(R"(
        Returns an iterator pointing one beyond the last element of an iterable
        sequence.
    )")
iEnd
...
iBegin(operator_::Equal, ".op.equal")
    ...
    iDoc(R"(
        Compares two values of the same type, assuming the type supports
        comparision.
    )")
iEnd
...
iBegin(operator_::Unequal, "unequal")
    ...
    iDoc(R"(
        Compares two values of the same type, returning true if they don't
        match. This operator is defined for all types that offer an ``equal``
        operator.
     )")
iEnd
...
iBegin(operator_::Unpack, "unpack")
    ...
    iDoc(R"(
        ...
        *op2* defines the binary layout as an enum of type ``Hilti::Packed`` and
        ...
        layout. The operator returns a ``tuple<T, iterator<bytes>>``, in the
        first component is the newly unpacked instance and the second component
        is locates the first bytes that has *not* been consumed anymore.

        Raises ~~WouldBlock if there are not sufficient bytes available for
        unpacking the type. Can also raise ``UnpackError` if the raw bytes are
        not as expected (and that fact can be verified).

        Note: The ``unpack`` operator uses a generic implementation able to
        handle all data types. Different from most other operators, it's
        implementation is not overloaded on a per-type based. Instead, the type
        specific code is implemented in the codegen::Unpacker.
    )")
iEnd
...
iBegin(operator_::Pack, "pack")
    ...
    iDoc(R"(
        Packs a value *op1* of a particular type into binary data enclosed
        by the iterator tuple *op1*. *op2* defines the binary layout as an
        enum of type ``Hilti::Packed`` and must be a constant. Depending on
        *op2*, *op3* is may be an additional, format-specific parameter with
        further information about the binary layout. The operator returns a
        ``ref<bytes>`` with the packed data.

        Note: The ``unpack`` operator uses a generic implementation able to
        handle all data types. Different from most other operators, it's
        implementation is not overloaded on a per-type based. Instead, the type
        specific code is implemented in the codegen::Packer.
    )")
iEnd
