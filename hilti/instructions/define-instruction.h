...
#define IMPLEMENT_INSTRUCTION(ns, cls, id)                                                         \
    void __register_##ns##_##cls()                                                                 \
    {                                                                                              \
        ::hilti::instruction::ns::cls =                                                            \
            std::make_shared<::hilti::instruction::ns::__class::cls>();                            \
        InstructionRegistry::globalRegistry()->addInstruction(::hilti::instruction::ns::cls);      \
    }                                                                                              \
    AST_RTTI_BEGIN(hilti::instruction::ns::__class::cls, id##_1)                                   \
    AST_RTTI_PARENT(hilti::Instruction)                                                            \
    AST_RTTI_END(id##_1)                                                                           \
    AST_RTTI_CAST_BEGIN(hilti::instruction::ns::__class::cls)                                      \
    AST_RTTI_CAST_PARENT(hilti::Instruction)                                                       \
    AST_RTTI_CAST_END()                                                                            \
                                                                                                   \
    AST_RTTI_BEGIN(hilti::statement::instruction::ns::cls, id##_2)                                 \
    AST_RTTI_PARENT(hilti::statement::instruction::Resolved)                                       \
    AST_RTTI_END(id##_2)                                                                           \
    AST_RTTI_CAST_BEGIN(hilti::statement::instruction::ns::cls)                                    \
...
#define iBegin(ns, cls, name)                                                                      \
    ...
    namespace ns {                                                                                 \
    class cls : public hilti::statement::instruction::Resolved {                                   \
        AST_RTTI                                                                                   \
    public:                                                                                        \
        cls(shared_ptr<::hilti::Instruction> instruction, const hilti::instruction::Operands& ops, \
    ...
    class cls : public hilti::Instruction {                                                        \
        AST_RTTI                                                                                   \
    public:                                                                                        \
        static shared_ptr<statement::instruction::Resolved> factory(                               \
            shared_ptr<hilti::Instruction> instruction, const hilti::instruction::Operands& ops,   \
            const Location& l = Location::None)                                                    \
        {                                                                                          \
            return shared_ptr<statement::instruction::Resolved>(                                   \
                new typename hilti::statement::instruction::ns::cls(instruction, ops, l));         \
        }                                                                                          \
...
#define __implementOp(nr, ty, constant)                                                            \
    ...
    {                                                                                              \
        if ( ! op )                                                                                \
            return __defaultOp##nr() || ast::rtti::isA<type::OptionalArgument>(ty);                \
        if ( coerce ) {                                                                            \
            if ( ! op->canCoerceTo(ty) )                                                           \
                return false;                                                                      \
        }                                                                                          \
        else if ( ! ty->equal(op->type()) )                                                        \
            return false;                                                                          \
        return true;                                                                               \
    }
...
#define iTerminator()                                                                              \
    std::set<shared_ptr<Expression>> __successors(const hilti::instruction::Operands& ops)         \
        const override;                                                                            \
...
#define iSuccessorsCC(cls)                                                                         \
    ...
    std::set<shared_ptr<Expression>> cls::__successors(const hilti::instruction::Operands& ops)    \
        const                                                                                      \
    {                                                                                              \
        shared_ptr<Expression> target = __get_op(0);                                               \
        shared_ptr<Expression> op1 = __get_op(1);                                                  \
...
/// Override flowInfo().
#define iFlowInfoH() FlowInfo __flowInfo(FlowInfo fi) override
