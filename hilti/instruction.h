...
class Instruction : public Node, public InstructionHelper {
    AST_RTTI
public:
    typedef shared_ptr<statement::instruction::Resolved> (*stmt_factory)(
        shared_ptr<hilti::Instruction> instruction, const instruction::Operands& ops,
        const Location& l);
    ...
    bool hideInDebugTrace() const
    {
        return __hideInDebugTrace();
    }
    ...
    std::set<shared_ptr<statement::Block>> successors(const hilti::instruction::Operands& ops,
    ...
    bool checkCallParameters(shared_ptr<type::Function> func, shared_ptr<Expression> args,
                             bool allow_unbound = false) const;
    ...
    FlowInfo flowInfo(FlowInfo fi)
    {
        return __flowInfo(fi);
    }
...
protected:
    ...
    // For internal use only. Will be overridden automagically via macros.
    virtual const char* __doc() const
    {
        return "No documentation.";
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual bool __matchOp0(shared_ptr<Expression> op, bool coerce)
    {
        return op.get() == nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual bool __matchOp1(shared_ptr<Expression> op, bool coerce)
    {
        return op.get() == nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual bool __matchOp2(shared_ptr<Expression> op, bool coerce)
    {
        return op.get() == nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual bool __matchOp3(shared_ptr<Expression> op, bool coerce)
    {
        return op.get() == nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual std::pair<shared_ptr<Type>, bool> __typeOp0() const
    {
        return std::make_pair(nullptr, false);
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual std::pair<shared_ptr<Type>, bool> __typeOp1() const
    {
        return std::make_pair(nullptr, false);
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual std::pair<shared_ptr<Type>, bool> __typeOp2() const
    {
        return std::make_pair(nullptr, false);
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual std::pair<shared_ptr<Type>, bool> __typeOp3() const
    {
        return std::make_pair(nullptr, false);
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual shared_ptr<Expression> __defaultOp0() const
    {
        return nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual shared_ptr<Expression> __defaultOp1() const
    {
        return nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual shared_ptr<Expression> __defaultOp2() const
    {
        return nullptr;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual shared_ptr<Expression> __defaultOp3() const
    {
        return nullptr;
    }

    virtual bool __hideInDebugTrace() const
    {
        return false;
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual std::set<shared_ptr<Expression>> __successors(
        const hilti::instruction::Operands& ops) const
    {
        return std::set<shared_ptr<Expression>>();
    }

    // For internal use only. Will be overridden automagically via macros.
    virtual bool __terminator() const
    {
        return false;
    }

    // For internal use only. May adapt the flowinfo.
    virtual FlowInfo __flowInfo(FlowInfo fi)
    {
        return fi;
    };
...
public:
    ...
    {
        return _instructions.find(name) != _instructions.end();
    }
    ...
    shared_ptr<statement::instruction::Resolved> resolveStatement(
    ...
    /// ops: The operands.
    shared_ptr<statement::instruction::Resolved> resolveStatement(
        shared_ptr<Instruction> instr, const instruction::Operands& ops,
