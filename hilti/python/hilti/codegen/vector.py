...
_doc_c_conversion = """
A ``vector`` is mapped to a ``__hlt_vector *``.
"""

@codegen.typeInfo(type.Vector)
def _(type):
    typeinfo = codegen.TypeInfo(type)
    typeinfo.c_prototype = "__hlt_vector *"
    return typeinfo
...
def _(type):
    typeinfo = codegen.TypeInfo(type)
    typeinfo.c_prototype = "__hlt_vector_iter"
    return typeinfo
...
def _(self, i):
    ...
    result = self.llvmGenerateCCallByName("__Hlt::vector_new", [self.llvmConstDefaultValue(t)], [t], llvm_args=True)
    self.llvmStoreInTarget(i.target(), result)

@codegen.when(instructions.vector.Get)
def _(self, i):
    t = i.op1().type().refType().itemType()
    voidp = self.llvmGenerateCCallByName("__Hlt::vector_get", [i.op1(), i.op2()])
...
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::vector_set", [i.op1(), i.op2(), i.op3()])

@codegen.when(instructions.vector.Size)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::vector_size", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.when(instructions.vector.Reserve)
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::vector_reserve", [i.op1(), i.op2()])

@codegen.when(instructions.vector.Begin)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::vector_begin", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)
    
@codegen.when(instructions.vector.End)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::vector_end", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)
    
@codegen.operator(instructions.vector.IterIncr)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::vector_iter_incr", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.operator(instructions.vector.IterDeref)
def _(self, i):
    t = i.target().type()
    voidp = self.llvmGenerateCCallByName("__Hlt::vector_iter_deref", [i.op1()])
    casted = codegen.builder().bitcast(voidp, llvm.core.Type.pointer(codegen.llvmTypeConvert(t)))
    self.llvmStoreInTarget(i.target(), self.builder().load(casted))
    
@codegen.operator(instructions.vector.IterEqual)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::vector_iter_eq", [i.op1(), i.op2()])
    self.llvmStoreInTarget(i.target(), result)
