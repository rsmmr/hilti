...
_doc_c_conversion = """A ``string`` is mapped to a ``hlt_string``."""
...
def _(self, i):
    result = self.llvmGenerateCCallByName("hlt::string_concat", [i.op1(), i.op2()], [i.op1().type(), i.op2().type()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.when(instructions.string.Cmp)
def _(self, i):
    result = self.llvmGenerateCCallByName("hlt::string_cmp", [i.op1(), i.op2()], [i.op1().type(), i.op2().type()])
...
def _(self, i):
    result = self.llvmGenerateCCallByName("hlt::string_encode", [i.op1(), i.op2()], [i.op1().type(), i.op2().type()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.when(instructions.string.Decode)
def _(self, i):
    result = self.llvmGenerateCCallByName("hlt::string_decode", [i.op1(), i.op2()], [i.op1().type(), i.op2().type()])
    self.llvmStoreInTarget(i.target(), result)
