...
_doc_c_conversion = """
A ``list`` is mapped to a ``__hlt_list *``.
"""

@codegen.typeInfo(type.List)
def _(type):
    typeinfo = codegen.TypeInfo(type)
    typeinfo.c_prototype = "__hlt_list *"
    typeinfo.to_string = "__Hlt::list_to_string"
    return typeinfo
...
def _(type):
    typeinfo = codegen.TypeInfo(type)
    typeinfo.c_prototype = "__hlt_list_iter"
    return typeinfo
...
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::list_push_front", [i.op1(), i.op2()])

@codegen.when(instructions.list.PushBack)
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::list_push_back", [i.op1(), i.op2()])

@codegen.when(instructions.list.PopFront)
def _(self, i):
    t = i.op1().type().refType().itemType()
    voidp = self.llvmGenerateCCallByName("__Hlt::list_pop_front", [i.op1()])
...
def _(self, i):
    t = i.op1().type().refType().itemType()
    voidp = self.llvmGenerateCCallByName("__Hlt::list_pop_back", [i.op1()])
    casted = codegen.builder().bitcast(voidp, llvm.core.Type.pointer(codegen.llvmTypeConvert(t)))
    self.llvmStoreInTarget(i.target(), self.builder().load(casted))
...
def _(self, i):
    t = i.op1().type().refType().itemType()
    voidp = self.llvmGenerateCCallByName("__Hlt::list_front", [i.op1()])
    casted = codegen.builder().bitcast(voidp, llvm.core.Type.pointer(codegen.llvmTypeConvert(t)))
    self.llvmStoreInTarget(i.target(), self.builder().load(casted))
...
def _(self, i):
    t = i.op1().type().refType().itemType()
    voidp = self.llvmGenerateCCallByName("__Hlt::list_back", [i.op1()])
    casted = codegen.builder().bitcast(voidp, llvm.core.Type.pointer(codegen.llvmTypeConvert(t)))
    self.llvmStoreInTarget(i.target(), self.builder().load(casted))

@codegen.when(instructions.list.Size)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::list_size", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.when(instructions.list.Erase)
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::list_erase", [i.op1()])

@codegen.when(instructions.list.Insert)
def _(self, i):
    self.llvmGenerateCCallByName("__Hlt::list_insert", [i.op1(), i.op2()])
    
@codegen.when(instructions.list.Begin)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::list_begin", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)
    
@codegen.when(instructions.list.End)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::list_end", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)
    
@codegen.operator(instructions.list.IterIncr)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::list_iter_incr", [i.op1()])
    self.llvmStoreInTarget(i.target(), result)

@codegen.operator(instructions.list.IterDeref)
def _(self, i):
    t = i.target().type()
    voidp = self.llvmGenerateCCallByName("__Hlt::list_iter_deref", [i.op1()])
    casted = codegen.builder().bitcast(voidp, llvm.core.Type.pointer(codegen.llvmTypeConvert(t)))
    self.llvmStoreInTarget(i.target(), self.builder().load(casted))
    
@codegen.operator(instructions.list.IterEqual)
def _(self, i):
    result = self.llvmGenerateCCallByName("__Hlt::list_iter_eq", [i.op1(), i.op2()])
    self.llvmStoreInTarget(i.target(), result)
