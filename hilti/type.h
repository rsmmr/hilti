...
public:
    ...
    virtual shared_ptr<hilti::Scope> typeScope()
    {
        return nullptr;
    }
...
public:
    ...
    Type(shared_ptr<hilti::Type> type)
    {
        _type = type;
    }

    /// Returns the type the parameter specifies.
    shared_ptr<hilti::Type> type() const
    {
        return _type;
    }
...
public:
    ...
    /// an empty list.
    virtual type_list alsoMatchableTo() const
    {
        return type_list();
    }
};
...
template<typename T>
inline bool hasTrait(const hilti::Type* t) { return ast::rtti::isA<T>(t); }

/// Returns true if type \a t has trait \a T.
template<typename T>
inline bool hasTrait(shared_ptr<hilti::Type> t) { return ast::rtti::isA<T>(t); }

/// Dynamic-casts type \a t into trait class \a T.
template<typename T>
inline T* asTrait(hilti::Type* t) { return dynamicCast(t, T*); }

/// Dynamic-casts type \a t into trait class \a T.
template<typename T>
inline const T* asTrait(const hilti::Type* t) { return dynamicCast(t, const T*); }

/// Dynamic-casts type \a t into trait class \a T.
template<typename T>
inline T* asTrait(shared_ptr<hilti::Type> t) { return dynamicPointerCast(t, T); }
...
public:
    ...
    shared_ptr<hilti::Type> argType() const
    {
        return _arg;
    }
...
public:
    ...
    ///
    /// l: Associated location.
    Any(const Location& l = Location::None) : hilti::Type(l)
    {
        setMatchesAny(true);
    }
    virtual ~Any();
...
public:
    ...
    Unknown(shared_ptr<ID> id, const Location& l = Location::None) : hilti::Type(l)
    {
        _id = id;
        addChild(_id);
    }
...
public:
    ...
    bool _equal(shared_ptr<hilti::Type> other) const override
    {
        auto o = ast::rtti::checkedCast<TypeByName>(other.get());
        return name() == o->name();
    }
...
class Address : public ValueType,
                public trait::Atomic,
                public trait::Unpackable,
                public trait::Classifiable {
    AST_RTTI
public:
    /// Constructor.
    ///
...
public:
    ...
    shared_ptr<hilti::Type> typeType() const
    {
        return _rtype;
    }
    ...
    bool _equal(shared_ptr<hilti::Type> other) const override
    {
        auto o = ast::rtti::checkedCast<TypeType>(other.get());
        return _rtype->equal(o->_rtype);
    }
...
public:
    ...
    {
        return _libtype == "hlt_exception_unspecified";
    }
...
public:
    ...
    Parameter(shared_ptr<hilti::ID> id, shared_ptr<Type> type, bool constant,
...
public:
    ...
    hilti::type::function::CallingConvention callingConvention() const
    {
        return _cc;
    }

    /// Sets the function's calling convention.
    void setCallingConvention(hilti::type::function::CallingConvention cc)
    {
        _cc = cc;
    }
    ...
    void setCcPlusOne(bool plusone)
    {
        _plusone = plusone;
    }
...
public:
    ...
    ///
    /// l: Associated location.
    HiltiFunction(shared_ptr<hilti::type::function::Result> result,
                  const function::parameter_list& args, hilti::type::function::CallingConvention cc,
...
///
class Callable : public Function,
                 public trait::HeapType,
                 public trait::GarbageCollected,
                 public trait::Parameterized {
    AST_RTTI
public:
    /// Constructor.
...
///
class Bytes : public HeapType,
              public trait::Iterable,
              public trait::Unpackable,
              public trait::Hashable,
              public trait::Classifiable,
              public trait::Blockable {
    AST_RTTI
public:
    /// Constructor.
    ///
...
public:
    ...
    shared_ptr<Type> ruleType() const
    {
        return _rtype;
    }

    /// Returns the type for value.
    shared_ptr<Type> valueType() const
    {
        return _vtype;
    }
...
public:
    ...
    ///
    /// l: Associated location.
    Field(shared_ptr<ID> name, shared_ptr<Type> type, int start, shared_ptr<Expression> fmt,
          shared_ptr<Expression> arg = nullptr, const Location& l = Location::None);
    ...
    ///
    /// l: Associated location.
    Field(shared_ptr<ID> name, shared_ptr<Type> type, shared_ptr<ID> start,
          shared_ptr<Expression> fmt, shared_ptr<Expression> arg = nullptr,
          const Location& l = Location::None);
    ...
    shared_ptr<ID> startField() const
    {
        return _start_field;
    }
    ...
    const field_list& dependents() const
    {
        return _deps;
    }
...
public:
    ...
    /// l: Associated location.
    ///
    IOSource(shared_ptr<Type> ty, const Location& l = Location::None) : Iterator(ty, l)
    {
    }
    ...
    /// l: Associated location.
    ///
    IOSource(const Location& l = Location::None) : IOSource{std::make_shared<type::IOSource>(l), l}
    {
    }
...
public:
    ...
    Vector(shared_ptr<Type> ctype, const Location& l = Location::None)
        : ContainerIterator<Vector>(ctype, l)
    {
    }
    ACCEPT_VISITOR(Iterator);
};
...
public:
    ...
    List(shared_ptr<Type> ctype, const Location& l = Location::None)
        : ContainerIterator<List>(ctype, l)
    {
    }
    ACCEPT_VISITOR(Iterator);
};
...
public:
    ...
    ///
    /// l: Associated location.
    List(shared_ptr<Type> etype, const Location& l = Location::None) : TypedHeapType(etype, l)
    {
    }
...
public:
    ...
    ///
    /// l: Associated location.
    Vector(shared_ptr<Type> etype, const Location& l = Location::None) : TypedHeapType(etype, l)
    {
    }
...
public:
    ...
    ///
    /// l: Associated location.
    Set(shared_ptr<Type> etype, const Location& l = Location::None) : TypedHeapType(etype, l)
    {
    }
...
public:
    ...
    Field(shared_ptr<ID> id, shared_ptr<hilti::Type> type, bool internal = false,
          const Location& l = Location::None);
...
/// Type for structs.
class Struct : public HeapType,
               public trait::TypeList,
               public trait::Hashable,
               public trait::Parameterized {
    AST_RTTI
public:
...
public:
    ...
    ///
    /// l: Associated location.
    Context(const Location& l = Location::None) : Struct(l)
    {
    }
...
/// Type for unions.
class Union : public ValueType, public trait::TypeList, public trait::Parameterized {
    AST_RTTI
public:
